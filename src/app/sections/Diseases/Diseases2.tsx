...
export default function DiseasesSection2() {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-dw-dark/80" />
      <div className="container mx-auto px-4 relative z-10">
        <h2 className="text-3xl mb-2 text-center text-white">{content.diseasesSection.sectionTitle}</h2>
            ...
            {displayedDiseases.map((disease, index) => (
              <button
                ...
                className={`py-4 px-2 sm:px-6 text-center font-semibold transition-colors
                  ${index < 3 ? "w-1/3" : "w-1/2"}
                  ${activeTab === index ? "bg-dw-darker text-white" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}
              >
                <div className="text-xl md:text-base">{disease.name}</div>
